...
import { AD_INTERVAL_MS, AD_VISIBLE_MS, NATIVE_AD_UNIT_ID } from "@/constants/ads";
...
export function PlayerArtworkAd({ uri, seed, size, radius, style }: Props) {
  ...
  useEffect(() => {
    // No ads while disabled or inside a "remove ads" reward window.
    if (!adsEnabled) {
      setShowAd(false);
      return;
    }
